function runTrackerDect(file_path)
...
%            rr = pos(1,2);
%            key = 0;
%             for ix = cc - target_size(1,2)/2 : cc + target_size(1,2)/2
%                 for iy = rr - target_size(1,1)/2 : rr + target_size(1,1)/2
%                     
%                     pos1 = [iy ix];
%                     patch = getPatch(img, pos1, app_model_patch_size);
%                     zf_t = fft2(computeFeatures(patch, cell_size, []));
%                     [~,max_response] = getNewPos(zf_t, xf_t, A_t);
%                 
%                     if max_response > 0.3
...
%             end
            ...
            cc = size(img,2) - target_size(1,2)/2;
            key = 0;
            for rr = target_size(1,1)/2 : size(img,1)- target_size(1,1)/2
                pos1 = [rr cc];
                patch = getPatch(img, pos1, app_model_patch_size);
                zf_t = fft2(computeFeatures(patch, cell_size, []));
                [~,max_response] = getNewPos(zf_t, xf_t, A_t);
