...
function [feat, pos_samples, labels, weights] =  det_samplesConv(im, pos,motion_model_patch_size,det_config,net_motion)

patch = getPatch(im, pos, floor(motion_model_patch_size*1.2));
%w_area = get_subwindow(im, pos, floor(window_sz*1.2));

%feat = get_feature_detector(patch, det_config.nbin);

patch = imresize(patch,4,'nearest');
feat = computeConvFeatures(patch, net_motion);
